...
**判断点・未確認点を残したままタスクを作成しない。** 「この会話を知らない別環境の Claude Code が単独で実行できる粒度」で書くと定めている以上、判断が残っていればその前提が崩れる。

作成前に、判断が要る点と未確認の点を洗い出して仕分ける。

| 区分 | 内容 | 扱い |
|---|---|---|
| **A** | 設計の骨格を左右するもの（どの方式を採るかが変わる） | **作成前に潰す。** 判断はユーザーに確認し、事実は実測する |
| **B** | 実装中に判明すればよいもの（パラメータ値、細部の挙動） | 「未確認点」に残してよい |

**A が1つでも残っているタスクは作成しない。**

判断保留を示す語（`要判断` / `または` / `〜でよい` / `〜が適切` / `見込み` / `どちらか` / `検討する`）が決定事項・受け入れ条件に混ざっていないか、作成前に検索して確認する。
