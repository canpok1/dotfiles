...
`brain/tasks/view.base` の「未完了」ビューは `status != "done"` フィルタのため、`dropped` を除外するよう修正する。修正しないと破棄済みタスクが未完了として残り続ける。
...
- **`done` を流用する**: `brain-task.md` の「やらないことにした受け入れ条件は打ち消し線を引き `[x]` 判定から除外する」仕組みを使えば形式的には成立し、`view.base` の変更も要らない。しかし `done` が「やり切った」と「やめた」を兼ね、ADR 0005 で解消した「`todo` が2つの意味を兼ねる」問題を作り直すため却下
